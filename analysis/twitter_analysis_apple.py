# -*- coding: utf-8 -*-
"""twitter_analysis_Apple.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1AmWXz14sxf9rWXGVIVg1II6NFcXT0x-F
"""

import pandas as pd

df = pd.read_csv('/content/AppleSupport_tweets.csv')
display(df.head())
...
"""# Task
Analyze the file and build the conversations.
...
# Extract the year from 'created_at' and group by year
grouped_by_year = df.groupby(df['created_at'].dt.year).size().reset_index(name='count')

# Rename the columns for clarity
grouped_by_year.columns = ['year', 'message_count']
...
# Extract the month from 'created_at' and group by month
grouped_by_month = df.groupby(df['created_at'].dt.month).size().reset_index(name='count')
...
output_filename = f'Apple_100_Nov2017_{formatted_time}.json'
...
# Filter conversations for November 2017
nov_2017_conversations = []
for conversation in structured_conversations:
    # Check if any tweet in the conversation is from Nov 2017
    if any(isinstance(tweet['created_at'], (datetime, pd.Timestamp)) and
           tweet['created_at'].year == 2017 and
           tweet['created_at'].month == 11
           for tweet in conversation['tweets']):
        nov_2017_conversations.append(conversation)


# Select 100 random conversations from the filtered list
if len(nov_2017_conversations) >= 100:
    random_nov_2017_conversations = random.sample(nov_2017_conversations, 100)
else:
    random_nov_2017_conversations = nov_2017_conversations
    print(f"Warning: Only {len(nov_2017_conversations)} conversations found for November 2017. Saving all of them.")


# Save the selected conversations to a JSON file
with open(output_filename, 'w') as f:
    json.dump(random_nov_2017_conversations, f, indent=4, cls=NpEncoder)

print(f"Selected {len(random_nov_2017_conversations)} random conversations from November 2017 saved to '{output_filename}'")